...
!
!-- nSpec    is >= the total number of species
!-- nReac    is >= the number of reactions
...
!
      PARAMETER(NOP=2331)
!
    !   CHARACTER*8  RE1(nReac),RE2(nReac),P1(nReac),P2(nReac),P3(nReac),P4(nReac), &
    !               & specname(nSpec),SPX(NEL)
      CHARACTER*25 UNIT2,UNIT3,UNIT7,UNIT8,DGFILE,TENFILE,RHONFILE, &
            & RHOEFILE, RHOIFILE, UNITL
      REAL*8  Y0(nSpec), RATE(nReac),TRAT(nReac,4),PRAT(nReac,4), &
            & DRAT(nReac,4),ERAT(nReac,4),B(nSpec,NOP), TAGE(NOP)
...
!*& For LSODE
      ...
      EXTERNAL DIFFUN
!**
      COMMON/BLK1/specname,nReac,nSpec
      COMMON/BLK2/B
      COMMON/BLK3/X,TOTAL,RATE
...
!       READ(7,1)(INDXS,specname(J+2),J=1,NTD)
...
!
      DO J=1,nReac
        IF(RE2(J).EQ.'CRP') THEN
            RATE(J)=GAMA(J)*CRAT
        ELSE IF ((RE2(J).EQ.'PHOTON')) THEN
            IPR=IPR+1
            IF (beta(J) .EQ. 0.0) BETA=DEFF
            PRAT(IPR,1)=INDXJ
            PRAT(IPR,2)=GAMA(J)
            PRAT(IPR,3)=ALPHA(J)
            PRAT(IPR,4)=BETA(J)*ALBFAC*CRAT 
    ...
    !--Identify reaction number for: H + grain -> H2
        ...
        ELSE IF((RE2(J).EQ.'CRPHOT')) THEN
            IER=IER+1
            ERAT(IER,1)=INDXJ
            IF (GAMA(J) .EQ. 0.0) GAMA(J)=CRDEF
            ERAT(IER,2)=GAMA(J)
            ERAT(IER,3)=ALPHA(J)
            ERAT(IER,4)=BETA(J)
        ELSE IF ((ALPHA(J) .NE. 0.0) .OR. (BETA(J) .NE. 0.0)) THEN
            ITR=ITR+1
            TRAT(ITR,1)=INDXJ
            TRAT(ITR,2)=GAMA(J)
            TRAT(ITR,3)=ALPHA(J)
...
!******************************************************************************
    ...
    !--Set the initial conditions for elemental abindances and other important molecules
    !--according to their published abundances
      ...
      ! Everything else set to published abundance
      Y0(nh2) = 0.5*(0.5*(1.0e0-fh))
      Y0(nh) = (0.5*(1.0e0-fh))     
      Y0(nhe) = fhe                       
      Y0(no) = fo  
      Y0(nn) = fn
    ...
    !   IF (np.ne.0) Y0(np) = fp ! If statement catches Phosphorous not being in network
    !   IF (nf.ne.0) Y0(nf) = ff ! Same here with Fluorine
!
...
!
 46   CLOSE(2)
      CALL RESULT(IRUN,nSpec,IFULL,LOUT,TAGE)
!--     IF(IPLOT.EQ.1) CALL PLOT(IRUN,nSpec)
!--     IF(IPLOT.EQ.0) CALL PLOT1(IRUN,nSpec)
...
!
      ...
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      PARAMETER(NCONS=2,nReac=1500)
!
    !   REAL*8 RATE(nReac),Y(N),YDOT(N),X(NCONS),TOTAL(NCONS), &
    !  &       TRAT(nReac,4),PRAT(nReac,4),DRAT(nReac,4),ERAT(nReac,4)
    !   COMMON/BLK3/X,TOTAL,RATE
    !   COMMON/BLK4/TRAT,PRAT,DRAT,ERAT,ITR,IPR,IDR,IER,IH2
    !   COMMON/BLK5/IFC,ICOV,ISURF
    !   COMMON/BLK6/DEN0,TEMP0,AV0
...
!          DO 5 I=1,IPR
!              INDXJ=PRAT(I,1)
!              RATE(INDXJ)=G0*PRAT(I,2)*DEXP(-AV*PRAT(I,3))+PRAT(I,4)
!  5       CONTINUE
! !--H2 and CO self-shielding factors
!          RATE(1059)=H2SHL*RATE(1059)
!          RATE(1066)=COSHL*RATE(1066)
...
! !
!             RATE(IND)=0.0
!             IF(ALPHA.EQ.0.0) RATE(IND)=GRAT0/DSQRT(BETA)
!             IF(ALPHA.EQ.-1.0) RATE(IND)=GRATN/DSQRT(BETA)
!             IF(ALPHA.EQ.1.0) RATE(IND)=GRATP/DSQRT(BETA)
! !--Apply freeze-out rules (to switch off rates)
! !--          IF((AV.LT.AVCRIT).AND.(GAMMA.EQ.4.0)) RATE(IND)=0.0
! !--          IF((AV.GT.AVCRIT).AND.(GAMMA.EQ.3.0)) RATE(IND)=0.0
!             IF(GAMMA.EQ.3.0) RATE(IND)=0.0
!  2       CONTINUE
...
!          RATE(1231)=FCO2*COCOV*RATE(1231)
!          RATE(1232)=FCO2*COCOV*RATE(1232)
!          RATE(1233)=FCO2*COCOV*RATE(1233)
! !--conversion of O,O+ and OH+ to H2O
!          RATE(1143)=FOX*(1.0-(FCO2*COCOV))*RATE(1143)
!          RATE(1145)=FOX*(1.0-(FCO2*COCOV))*RATE(1145)
!          RATE(1210)=FOX*(1.0-(FCO2*COCOV))*RATE(1210)
...
!
      INTEGER*8 nReac
      DIMENSION B(nSpec,NOP),BL(nSpec,NOP),TAGE(NOP)
      CHARACTER*8 specname(nSpec)
...
!---Full output of all abundances
         IST=1
         IFN=8
 4       WRITE(LOUT,104) (specname(J),J=IST,IFN)
         WRITE(LOUT,101)
         DO 5 I=1,IRUN
...
!---Reduced output
      ELSE IF(IFULL.EQ.0) THEN
       WRITE(LOUT,104) specname(J1),specname(J2),specname(J3),specname(J4), &
     &   specname(J5),specname(J6),specname(J7),specname(J8)
       WRITE(LOUT,101)
       DO 6 I=1,IRUN
...
!
      INTEGER*8 nReac
      DIMENSION RATE(nReac),Y(N),YDOT(N)
      COMMON/BLK1/specname,nReac,nSpec
      COMMON /BLK3/RATE,D
      COMMON /BLK9/FCO2,COCOV,FOX,PHOH,H2SHL,COSHL,FFRZ,FDES
!
