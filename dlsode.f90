!------------------------------------------------------------------------------
! N.I.M.R.O.D. - Normal approximation Inference in Models with Random
!                effects based on Ordinary Differential equations
!------------------------------------------------------------------------------
!
! VERSION : 1.0
!
...
!C19791129  DATE WRITTEN
!C19791213  Minor changes to declarations; DELP init. in STODE.
!C19800118  Treat NEQ as array; integer declarations added throughout;
!C          minor changes to prologue.
!C19800306  Corrected TESCO(1,NQP1) setting in CFODE.
!C19800519  Corrected access of YH on forced order reduction;
!C          numerous corrections to prologues and other comments.
!C19800617  In main driver, added loading of SQRT(UROUND) in RWORK;
!C          minor corrections to main prologue.
!C19800923  Added zero initialization of HU and NQU.
!C19801218  Revised XERRWD routine; minor corrections to main prologue.
!C19810401  Minor changes to comments and an error message.
!C19810814  Numerous revisions: replaced EWT by 1/EWT; used flags
...
!C19870330  Major update by ACH: corrected comments throughout;
...
!C19890426  Modified prologue to SLATEC/LDOC format.  (FNF)
!C19890501  Many improvements to prologue.  (FNF)
!C19890503  A few final corrections to prologue.  (FNF)
!C19890504  Minor cosmetic changes.  (FNF)
!C19890510  Corrected description of Y in Arguments section.  (FNF)
!C19890517  Minor corrections to prologue.  (FNF)
!C19920514  Updated with prologue edited 891025 by G. Shaw for manual.
!C19920515  Converted source lines to upper case.  (FNF)
!C19920603  Revised XERRWD calls using mixed upper-lower case.  (ACH)
!C19920616  Revised prologue comment regarding CFT.  (ACH)
!C19921116  Revised prologue comments regarding Common.  (ACH).
!C19930326  Added comment about non-reentrancy.  (FNF)
!C19930723  Changed D1MACH to DUMACH. (FNF)
!C19930801  Removed ILLIN and NTREP from Common (affects driver logic);
...
!C19930809  Changed to generic intrinsic names; changed names of
!C          subprograms and Common blocks to DLSODE etc. (ACH)
!C19930929  Eliminated use of REAL intrinsic; other minor changes. (ACH)
!C20010412  Removed all 'own' variables from Common block /DLS001/
!C          (affects declarations in 6 routines). (ACH)
!C20010509  Minor corrections to prologue. (ACH)
!C20031105  Restored 'own' variables to Common block /DLS001/, to
!C          enable interrupt/restart feature. (ACH)
!C20031112  Added SAVE statements for data-loaded constants.
!C
!C**END PROLOGUE  DLSODE
!C
!CInternal Notes:
!C
!COther Routines in the DLSODE Package.
!C
!CIn addition to Subroutine DLSODE, the DLSODE package includes the
!Cfollowing subroutines and function routines:
...
!CNote: DVNORM, DUMACH, IXSAV, and IUMACH are function routines.
!CAll the others are subroutines.
!C
!C*End
!C
!C Declare externals.
...
!C----------------------------------------------------------------------
!CThe following internal Common block contains
!C(a) variables which are local to any subroutine but whose values must
!C    be preserved between calls to the routine ("own" variables), and
!C(b) variables which are communicated between subroutines.
!CThe block DLS001 is declared in subroutines DLSODE, DINTDY, DSTODE,
!CDPREPJ, and DSOLSY.
!CGroups of variables are replaced by dummy arrays in the Common
!Cdeclarations in routines where those variables are not used.
!C----------------------------------------------------------------------
...
!C----------------------------------------------------------------------
!CBlock A.
!CThis code block is executed on every call.
!CIt tests ISTATE and ITASK for legality and branches appropriately.
!CIf ISTATE .GT. 1 but the flag INIT shows that initialization has
!Cnot yet been done, an error return occurs.
!CIf ISTATE = 1 and TOUT = T, return immediately.
!C----------------------------------------------------------------------
!C
...
!C----------------------------------------------------------------------
!CBlock B.
!CThe next code block is executed for the initial call (ISTATE = 1),
!Cor for a continuation call with parameter changes (ISTATE = 3).
!CIt contains checking of all inputs and various initializations.
!C
!CFirst check legality of the non-optional inputs NEQ, ITOL, IOPT,
!CMF, ML, and MU.
!C----------------------------------------------------------------------
 ...
 30   CONTINUE
!CNext process and check the optional inputs. --------------------------
...
!C----------------------------------------------------------------------
!CSet work array pointers and check lengths LRW and LIW.
!CPointers to segments of RWORK and IWORK are named by prefixing L to
!Cthe name of the segment.  E.g., the segment YH starts at RWORK(LYH).
!CSegments of RWORK (in order) are denoted  YH, WM, EWT, SAVF, ACOR.
!C----------------------------------------------------------------------
 60   LYH = 21
...
!CIf ISTATE = 3, set flag to signal parameter changes to DSTODE. -------
      JSTART = -1
      IF (NQ .LE. MAXORD) GO TO 90
!CMAXORD was reduced below NQ.  Copy YH(*,MAXORD+2) into SAVF. ---------
      DO 80 I = 1,N
 80     RWORK(I+LSAVF-1) = RWORK(I+LWM-1)
!CReload WM(1) = RWORK(LWM), since LWM may have changed. ---------------
...
!C----------------------------------------------------------------------
!CBlock C.
!CThe next block is for the initial call only (ISTATE = 1).
!CIt contains all remaining initializations, the initial call to F,
!Cand the calculation of the initial step size.
!CThe error weights in EWT are inverted after being loaded.
!C----------------------------------------------------------------------
 100  UROUND = DUMACH()
      TN = T
...
!C----------------------------------------------------------------------
!CThe coding below computes the step size, H0, to be attempted on the
!Cfirst step, unless the user has supplied a value for this.
!CFirst check that TOUT - T differs significantly from zero.
!CA scalar tolerance quantity TOL is computed, as MAX(RTOL(I))
!Cif this is positive, or MAX(ATOL(I)/ABS(Y(I))) otherwise, adjusted
!Cso as to be between 100*UROUND and 1.0E-3.
!CThen the computed value H0 is given by..
!C                                     NEQ
!C  H0**2 = TOL / ( w0**-2 + (1/NEQ) * SUM ( f(i)/ywt(i) )**2  )
!C                                      1
!Cwhere   w0     = MAX ( ABS(T), ABS(TOUT) ),
!C        f(i)   = i-th component of initial value of f,
!C        ywt(i) = EWT(i)/TOL  (a weight for y(i)).
!CThe sign of H0 is inferred from the initial values of TOUT and T.
!C----------------------------------------------------------------------
...
!C----------------------------------------------------------------------
!CBlock E.
!CThe next block is normally executed for all calls and contains
!Cthe call to the one-step core integrator DSTODE.
!C
!CThis is a looping point for the integration steps.
!C
!CFirst check for too many steps being taken, update EWT (if not at
!Cstart of problem), check for too much accuracy being requested, and
!Ccheck for H below the roundoff level in T.
!C----------------------------------------------------------------------
 250  CONTINUE
...
!C----------------------------------------------------------------------
!CBlock F.
!CThe following block handles the case of a successful return from the
!Ccore integrator (KFLAG = 0).  Test for stop conditions.
!C----------------------------------------------------------------------
 300  INIT = 1
...
!CITASK = 5.  See if TCRIT was reached and jump to exit. ---------------
 350  HMX = DABS(TN) + ABS(H)
      IHIT = DABS(TN - TCRIT) .LE. 100.0D0*UROUND*HMX
!C----------------------------------------------------------------------
!CBlock G.
!CThe following block handles all successful returns from DLSODE.
...
!CThen Y is loaded from YH and T is set to TN.  The optional outputs
!Care loaded into the work arrays before returning.
!C----------------------------------------------------------------------
...
!C
 100  ELCO(1,1) = 1.0D0
      ...
      DO 140 NQ = 2,12
!C----------------------------------------------------------------------
!CThe PC array will contain the coefficients of the polynomial
...
!C----------------------------------------------------------------------
        ...
        FNQM1 = NQM1
        NQP1 = NQ + 1
!CForm coefficients of p(x)*(x+nq-1). ----------------------------------
        PC(NQ) = 0.0D0
        DO 110 IB = 1,NQM1
          I = NQP1 - IB
 110      PC(I) = PC(I-1) + FNQM1*PC(I)
        PC(1) = FNQM1*PC(1)
!CCompute integral, -1 to 0, of p(x) and x*p(x). -----------------------
        PINT = PC(1)
        XPIN = PC(1)/2.0D0
        TSIGN = 1.0D0
        DO 120 I = 2,NQ
...
!CStore coefficients in ELCO and TESCO. --------------------------------
...
!C----------------------------------------------------------------------
        FNQ = NQ
        NQP1 = NQ + 1
!CForm coefficients of p(x)*(x+nq). ------------------------------------
        PC(NQP1) = 0.0D0
        DO 210 IB = 1,NQ
          I = NQ + 2 - IB
...
!C**FIRST EXECUTABLE STATEMENT  DINTDY
      ...
      TP = TN - HU -  100.0D0*UROUND*SIGN(DABS(TN) + DABS(HU), HU)
...
!CIf MITER = 1, call JAC and multiply by scalar. -----------------------
 100  LENP = N*N
      DO 110 I = 1,LENP
 110    WM(I+2) = 0.0D0
 ...
 120    WM(I+2) = WM(I+2)*CON
      GO TO 240
!CIf MITER = 2, make N calls to F to approximate J. --------------------
 200  FAC = DVNORM (N, SAVF, EWT)
      R0 = 1000.0D0*DABS(H)*UROUND*N*FAC
 ...
 230    CONTINUE
      NFE = NFE + N
!CAdd identity matrix. -------------------------------------------------
 240  J = 3
      NP1 = N + 1
      DO 250 I = 1,N
        WM(J) = WM(J) + 1.0D0
 250    J = J + NP1
!CDo LU decomposition on P. --------------------------------------------
      CALL DGEFA (WM(3), N, N, IWM(21), IER)
      IF (IER .NE. 0) IERPJ = 1
      RETURN
!CIf MITER = 3, construct a diagonal approximation to J and P. ---------
 300  WM(2) = HL0
      R = EL0*0.1D0
      DO 310 I = 1,N
 ...
 330  IERPJ = 1
      RETURN
!CIf MITER = 4, call JAC and multiply by scalar. -----------------------
 400  ML = IWM(1)
      MU = IWM(2)
      ML3 = ML + 3
      MBAND = ML + MU + 1
 ...
 420    WM(I+2) = WM(I+2)*CON
      GO TO 570
!CIf MITER = 5, make MBAND calls to F to approximate J. ----------------
 500  ML = IWM(1)
      MU = IWM(2)
      MBAND = ML + MU + 1
      MBA = MIN(MBAND,N)
...
!C----------------------------------------------------------------------
!COn the first call, the order is set to 1, and other variables are
!Cinitialized.  RMAX is the maximum ratio by which H can be increased
!Cin a single step.  It is initially 1.E4 to compensate for the small
!Cinitial H, but then is normally equal to 10.  If a failure
!Coccurs (in corrector convergence or error test), RMAX is set to 2
!Cfor the next increase.
!C----------------------------------------------------------------------
      LMAX = MAXORD + 1
      NQ = 1
      L = 2
      ...
      GO TO 140
!C----------------------------------------------------------------------
!CThe following block handles preliminaries needed when JSTART = -1.
!CIPUP is set to MITER to force a matrix update.
!CIf an order increase is about to be considered (IALTH = 1),
!CIALTH is reset to 2 to postpone consideration one more step.
!CIf the caller has changed METH, DCFODE is called to reset
!Cthe coefficients of the method.
!CIf the caller has changed MAXORD to a value less than the current
!Corder NQ, NQ is reduced to MAXORD, and a new H chosen accordingly.
!CIf H is to be changed, YH must be rescaled.
!CIf H or METH is being changed, IALTH is reset to L = NQ + 1
!Cto prevent further changes in H for that many steps.
!C----------------------------------------------------------------------
 100  IPUP = MITER
      LMAX = MAXORD + 1
 ...
 125    EL(I) = ELCO(I,NQ)
      ...
      GO TO 175
!C----------------------------------------------------------------------
!CDCFODE is called to get all the integration coefficients for the
!Ccurrent METH.  Then the EL vector and related constants are reset
!Cwhenever the order NQ is changed, or at the start of the problem.
!C----------------------------------------------------------------------
...
!C----------------------------------------------------------------------
!CIf H is being changed, the H ratio RH is checked against
!CRMAX, HMIN, and HMXI, and the YH array rescaled.  IALTH is set to
!CL = NQ + 1 to prevent a change of H for that many steps, unless
!Cforced by a convergence or error test failure.
!C----------------------------------------------------------------------
...
!C----------------------------------------------------------------------
!CThis section computes the predicted values by effectively
!Cmultiplying the YH array by the Pascal Triangle matrix.
!CRC is the ratio of new to old values of the coefficient  H*EL(1).
!CWhen RC differs from 1 by more than CCMAX, IPUP is set to MITER
!Cto force PJAC to be called, if a Jacobian is involved.
!CIn any case, PJAC is called at least every MSBP steps.
!C----------------------------------------------------------------------
...
!Cir$ ivdep
 ...
 215    CONTINUE
!C----------------------------------------------------------------------
!CUp to MAXCOR corrector iterations are taken.  A convergence test is
!Cmade on the R.M.S. norm of each correction, weighted by the error
!Cweight vector EWT.  The sum of the corrections is accumulated in the
!Cvector ACOR(i).  The YH array is not altered in the corrector loop.
!C----------------------------------------------------------------------
 220  M = 0
      DO 230 I = 1,N
 230    Y(I) = YH(I,1)
...
!C----------------------------------------------------------------------
!CIf indicated, the matrix P = I - h*el(1)*J is reevaluated and
!Cpreprocessed before starting the corrector iteration.  IPUP is set
!Cto 0 as an indicator that this has been done.
!C----------------------------------------------------------------------
...
!C----------------------------------------------------------------------
!CIn the case of functional iteration, update Y directly from
!Cthe result of the last function evaluation.
!C----------------------------------------------------------------------
      DO 290 I = 1,N
 ...
 300    ACOR(I) = SAVF(I)
      GO TO 400
!C----------------------------------------------------------------------
!CIn the case of the chord method, compute the corrector error,
!Cand solve the linear system with that as right-hand side and
!CP as coefficient matrix.
!C----------------------------------------------------------------------
 350  DO 360 I = 1,N
...
!C----------------------------------------------------------------------
!CTest for convergence.  If M.gt.0, an estimate of the convergence
!Crate constant is stored in CRATE, and this is used in the test.
!C----------------------------------------------------------------------
 400  IF (M .NE. 0) CRATE = DMAX1(0.2D0*CRATE,DEL/DELP)
      ...
      GO TO 270
!C----------------------------------------------------------------------
!CThe corrector iteration failed to converge.
!CIf MITER .ne. 0 and the Jacobian is out of date, PJAC is called for
!Cthe next try.  Otherwise the YH array is retracted to its values
!Cbefore prediction, and H is reduced, if possible.  If H cannot be
!Creduced or MXNCF failures have occurred, exit with KFLAG = -2.
!C----------------------------------------------------------------------
...
!Cir$ ivdep
 ...
 445    CONTINUE
      ...
      GO TO 170
!C----------------------------------------------------------------------
!CThe corrector has converged.  JCUR is set to 0
!Cto signal that the Jacobian involved may need updating later.
!CThe local error test is made and control passes to statement 500
!Cif it fails.
!C----------------------------------------------------------------------
 450  JCUR = 0
...
!C----------------------------------------------------------------------
!CAfter a successful step, update the YH array.
!CConsider changing H if IALTH = 1.  Otherwise decrease IALTH by 1.
!CIf IALTH is then 1 and NQ .lt. MAXORD, then ACOR is saved for
!Cuse in a possible order increase on the next step.
!CIf a change in H is considered, an increase or decrease in order
!Cby one is considered also.  A change in H is made only if it is by a
!Cfactor of at least 1.1.  If not, IALTH is set to 3 to prevent
!Ctesting for that many steps.
!C----------------------------------------------------------------------
      KFLAG = 0
      IREDO = 0
      NST = NST + 1
 ...
 490    YH(I,LMAX) = ACOR(I)
      GO TO 700
!C----------------------------------------------------------------------
!CThe error test failed.  KFLAG keeps track of multiple failures.
!CRestore TN and the YH array to their previous values, and prepare
!Cto try the step again.  Compute the optimum step size for this or
!Cone lower order.  After 2 or more failures, H is forced to decrease
!Cby a factor of 0.2 or less.
!C----------------------------------------------------------------------
 500  KFLAG = KFLAG - 1
      TN = TOLD
      I1 = NQNYH + 1
...
!Cir$ ivdep
 ...
 515    CONTINUE
      ...
      GO TO 540
!C----------------------------------------------------------------------
!CRegardless of the success or failure of the step, factors
!CRHDN, RHSM, and RHUP are computed, by which H could be multiplied
!Cat order NQ - 1, order NQ, or order NQ + 1, respectively.
!CIn the case of failure, RHUP = 0.0 to avoid an order increase.
!CThe largest of these is determined and the new order chosen
!Caccordingly.  If the order is to be increased, we compute one
!Cadditional scaled derivative.
!C----------------------------------------------------------------------
 520  RHUP = 0.0D0
...
!C----------------------------------------------------------------------
!CIf there is a change of order, reset NQ, l, and the coefficients.
!CIn any case H is reset according to RH and the YH array is rescaled.
!CThen exit from 690 if the step was OK, or redo the step otherwise.
!C----------------------------------------------------------------------
 ...
 630  NQ = NEWQ
      ...
      GO TO 150
!C----------------------------------------------------------------------
!CControl reaches this section if 3 or more failures have occured.
!CIf 10 failures have occurred, exit with KFLAG = -1.
!CIt is assumed that the derivatives that have accumulated in the
!CYH array have errors of the wrong order.  Hence the first
!Cderivative is recomputed, and the order is set to 1.  Then
!CH is reduced by a factor of 10, and the step is retried,
!Cuntil it succeeds or H reaches HMIN.
!C----------------------------------------------------------------------
 ...
 650    YH(I,2) = H*SAVF(I)
      ...
      GO TO 150
!C----------------------------------------------------------------------
!CAll returns are made through this section.  H is saved in HOLD
!Cto allow the caller to change H on the next step.
!C----------------------------------------------------------------------
 660  KFLAG = -1
      GO TO 720
 670  KFLAG = -2
...
!CInternal Notes:
!C
!CFor a different default logical unit number, IXSAV (or a subsidiary
!Croutine that it calls) will need to be modified.
!CFor a different run-abort command, change the statement following
!Cstatement 100 at the end.
!C----------------------------------------------------------------------
!CSubroutines called by XERRWD.. None
!CFunction routine called by XERRWD.. IXSAV
!C----------------------------------------------------------------------
!C*End
!C
!C Declare arguments.
...
!C
!CSubroutines called by XSETF.. None
!CFunction routine called by XSETF.. IXSAV
!C----------------------------------------------------------------------
!C*End
...
!C
!CSubroutines called by XSETUN.. None
!CFunction routine called by XSETUN.. IXSAV
!C----------------------------------------------------------------------
!C*End
...
!C
!CSubroutines called by IXSAV.. None
!CFunction routine called by IXSAV.. IUMACH
!C----------------------------------------------------------------------
!C*End
      LOGICAL ISET
      INTEGER IPAR, IVALUE
...
!C----------------------------------------------------------------------
!CThe following Fortran-77 declaration is to cause the values of the
!Clisted (local) variables to be saved between calls to this routine.
!C----------------------------------------------------------------------
      SAVE LUNIT, MESFLG
